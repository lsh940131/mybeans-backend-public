...
import { ISA, ISAFile } from '../interface/seller-appl.interface';
import { ApiPropertyEnum } from '../../../../common/decorator/api-property-enum.decorator';

class SAFilePayload {
  constructor(data: ISAFile) {
    this.id = data.id;
    this.url = data.url;
  }
...
export class SAPayload {
  constructor(data: ISA) {
    this.status = data.status as SellerApplStatusEnum;
    this.step = data.step;
    ...
    this.fileList = data.sellerApplicationFile?.length
      ? data.sellerApplicationFile.map((v) => new SAFilePayload(v))
      : [];
  }

  @ApiPropertyEnum({
    description: '상태',
    enums: [{ name: 'SellerApplStatusEnum', enum: SellerApplStatusEnum }],
    default: SellerApplStatusEnum.TEMP,
  })
  @IsEnum(SellerApplStatusEnum)
  readonly status: SellerApplStatusEnum;
  ...
  // 첨부파일
  @ApiProperty({ description: '첨부파일 리스트', isArray: true, type: SAFilePayload })
  readonly fileList: SAFilePayload[];
}
